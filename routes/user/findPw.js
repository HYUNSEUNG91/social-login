...
router.post('/findPw', async (req, res) => {
    ...
    const emailOptions = {// 옵션값 설정
        ...
        html: "<h1 >마피양에서 새로운 비밀번호를 알려드립니다.</h1> <h2> 비밀번호 : " + randomPassword + "</h2>" +'<h3 style="color: crimson;">임시 비밀번호로 로그인 하신 후, 반드시 비밀번호를 수정해 주세요.</h3>',
    };
    ...
    const changePw = await User.findOneAndUpdate({userId:userId},
        {$set : {userPw : hashedPw}}, {new:true}) 
